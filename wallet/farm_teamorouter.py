...
async def harvest_one(email: str, mail_handle, use_proxy: bool = False) -> str:
    ...
    async with AsyncCamoufox(**kw) as fox:
        ...
        # the input. Note: the input sits inside a shadow root, so plain
        # document.querySelector JS hacks find nothing; always drive the
        # Playwright-resolved element (locators pierce shadow DOM).
        ...
        try:
            ...
            log(f"fill blocked ({type(e).__name__}) — JS native-setter on locator element")
            await el.evaluate("""(el, email) => {
        ...
        try:
            ...
            log(f"email field now: {filled[:6]}...{filled[-8:] if filled else '(EMPTY!)'}")
        except Exception:
            pass
